...
def encode_filename(filename):
    ...
    quoted = filename #urlquote(filename)
    if quoted == filename:
...
def render_to_pdf_2(template_name, context={}, encoding="utf-8"):
    template = get_template(template_name)
    html  = template.render(context)

    result = BytesIO()
    pdf = pisa.pisaDocument(BytesIO(html.encode(encoding)), result)
    if not pdf.err:
...
def generate_pdf(sourceHtml, outputFilename, encoding="utf-8"):
	# open output file for writing (truncated binary)
	resultFile = open(outputFilename, "w+b")
	# convert HTML to PDF
	# the HTML to convert [sourceHtml]
	pisaStatus = pisa.CreatePDF(sourceHtml,dest=resultFile, encoding=encoding)
	# close output file
	resultFile.close()
